...
def get_prediction_eos():
    try:
        ...
        input_text += ' <mask>'
        top_k = request.json['top_k']
        print(f"Received input_text for end prediction: {input_text}")  # Debugging log
        res = main.get_all_predictions(input_text, top_clean=int(top_k))
        return app.response_class(response=json.dumps(res), status=200, mimetype='application/json')
    except Exception as error:
        err = str(error)
        print(f"Error in get_end_predictions: {err}")
        return app.response_class(response=json.dumps(err), status=500, mimetype='application/json')
...
def get_prediction_mask():
    try:
        input_text = ' '.join(request.json['input_text'].split())
        top_k = request.json['top_k']
        print(f"Received input_text for mask prediction: {input_text}")  # Debugging log
        res = main.get_all_predictions(input_text, top_clean=int(top_k))
        return app.response_class(response=json.dumps(res), status=200, mimetype='application/json')
    except Exception as error:
        err = str(error)
        print(f"Error in get_mask_predictions: {err}")
        return app.response_class(response=json.dumps(err), status=500, mimetype='application/json')
